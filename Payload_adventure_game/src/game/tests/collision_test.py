...
class TestCollision(unittest.TestCase):
    ...
    def test_no_collision(self):
        self.assertEqual(self.collisions.are_colliding(self.player, self.circle), False)

    def test_remove_collision_without_collision(self):
        self.collisions.remove_collision(self.player, self.circle)
        self.assertEqual(self.player.rect == (0, 0, 20, 20), True)

    def test_collision(self):
        self.player.set_pos(290, 290)
        self.assertEqual(self.collisions.are_colliding(self.player, self.circle), True)
